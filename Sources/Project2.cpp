...
"Pressing T will change Track texture\n "
...
int main()
{
	...
	unsigned int heightmap_texture = loadTexture("../Project_2/Media/skybox_self/bottom.jpg");
	unsigned int diffuseMap = loadTexture("../Project_2/Media/textures/boxtexture1.jpg");
	unsigned int specularMap = loadTexture("../Project_2/Media/textures/boxtexture1.jpg");

	Track track("spline/track.sp");
	unsigned int rail_texture1 = loadTexture("../Project_2/Media/textures/wood.jpg");
	unsigned int rail_texture2 = loadTexture("../Project_2/Media/textures/marble.jpg");
	...
	Model houseModel("../Project_2/Media/Big_Old_House/Big_Old_House.obj");
	Model cartModel1("../Project_2/Media/Coca-Cola/Bottle Coca-Cola N080710.3ds");
	Model cartModel2("../Project_2/Media/lowpolydeer/deer.fbx");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindVertexArray(cubeVAO);
		for (unsigned int i = 1; i < track.controlPoints.size(); i++)
		{
			// calculate the model matrix for each object and pass it to shader before drawing
			glm::mat4 box_model;

			// Translate box for final offset
			box_model = glm::translate(box_model, glm::vec3(0.0f, 7.0f, 0.0f));
		...
		model = glm::translate(model, glm::vec3(18.0f, 0.0f, -10.0f)); // translate it down so it's at the center of the scene
		...
		lightingShader_nMap.use();
		lightingShader_nMap.setFloat("material.shininess", 16.0f);
		glm::mat4 housemodel_1;
		housemodel_1 = glm::translate(housemodel_1, glm::vec3(0.0f, -3.0f, -5.0f)); // translate it down so it's at the center of the scene
		//housemodel_1 = glm::rotate(housemodel_1, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f)); // Rotated it towards the light.  Praise the sun
		housemodel_1 = glm::scale(housemodel_1, glm::vec3(1.0f, 1.0f, 1.0f));	// it's a bit too big for our scene, so scale it down
		lightingShader_nMap.setMat4("model", housemodel_1);
		houseModel.Draw(lightingShader_nMap);

		lightingShader_nMap.use();
		lightingShader_nMap.setFloat("material.shininess", 16.0f);
		glm::mat4 housemodel_2;
		housemodel_2 = glm::translate(housemodel_2, glm::vec3(5.0f, -3.0f, 0.0f)); // translate it down so it's at the center of the scene
		housemodel_2 = glm::rotate(housemodel_2, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f)); // Rotated it towards the light.  Praise the sun
		housemodel_2 = glm::scale(housemodel_2, glm::vec3(1.0f, 1.0f, 1.0f));	// it's a bit too big for our scene, so scale it down
		lightingShader_nMap.setMat4("model", housemodel_2);
		houseModel.Draw(lightingShader_nMap);

		lightingShader_nMap.use();
		lightingShader_nMap.setFloat("material.shininess", 16.0f);
		glm::mat4 housemodel_3;
		housemodel_3 = glm::translate(housemodel_3, glm::vec3(-5.0f, -3.0f, 0.0f)); // translate it down so it's at the center of the scene
		housemodel_3 = glm::rotate(housemodel_3, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f)); // Rotated it towards the light.  Praise the sun
		housemodel_3 = glm::scale(housemodel_3, glm::vec3(1.0f, 1.0f, 1.0f));	// it's a bit too big for our scene, so scale it down
		lightingShader_nMap.setMat4("model", housemodel_3);
		houseModel.Draw(lightingShader_nMap);
		...
		{
			lightingShader_basic.use();
			track.Draw(lightingShader_basic, rail_texture1);
		}
		else if (drawTrack == 2)
		{
			lightingShader_basic.use();
			track.Draw(lightingShader_basic, rail_texture2);
		}
		else
		{
			reflectionShader.use();
			track.Draw(reflectionShader, rail_texture1);
		}
		...
		if (drawNormals)
		{
			...
			track.Draw(normalShader, specularMap);
		}
		if (freemode) {
		...
		if (trackmove) {
			...
			lightingShader_nMap.use();
			model_camera = glm::translate(model_camera, cartCamera.Position);
			glm::mat4 rotationMatrix;
			rotationMatrix = { glm::vec4(cartCamera.Right, 0), glm::vec4(cartCamera.Up, 0), glm::vec4(cartCamera.Front, 0), glm::vec4(0,0,0,1) };
			if (cart)
			{
			model_camera = model_camera * rotationMatrix;
			model_camera = glm::scale(model_camera, glm::vec3(0.01f, 0.01f, 0.01f));
			lightingShader_nMap.setMat4("model", model_camera);
			cartModel1.Draw(lightingShader_nMap);
			}
			else
			{
			model_camera = model_camera * rotationMatrix;
			model_camera = glm::scale(model_camera, glm::vec3(0.002f, 0.002f, 0.002f));
			model_camera = glm::rotate(model_camera, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
			model_camera = glm::rotate(model_camera, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
			lightingShader_nMap.setMat4("model", model_camera);
			cartModel2.Draw(lightingShader_nMap);
			}
...
void processInput(GLFWwindow *window)
{
	...
	if (somethingPressed && last_pressed < currentFrame - 0.5f || last_pressed == 0.0f)
	{
		...
		{
			if (drawTrack == 1) drawTrack = 2;
			else if (drawTrack == 2) drawTrack = 0;
			else drawTrack = 1;
		}
		...
		if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS)
		{
			if (trackmove) {
				...
				cartCamera.Up = glm::vec3(0.0f, 1.0f, 0.0f);
				cartCamera.Front = glm::vec3(0.0f, 0.0f, 1.0f);
				cartCamera.Right = glm::vec3(1.0f, 0.0f, 0.0f);
				cartCamera.Position = glm::vec3(0.0f, 0.0f, 0.0f);
				cartCamera.currentpos = 2;
				trackmove = true;
			}
		}
